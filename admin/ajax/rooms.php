<?php 
        ...
        while($row = mysqli_fetch_assoc($res)){
            ...
            $data.="
                <tr class='align-middle'>
                    ...
                    <td>$status</td>
                    <td>
                    <button type='button' onclick='edit_details($row[id])' class='btn btn-primary shadow-none bts-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
                        <i class='bi bi-pencil-square'></i>
                    </button>
                    <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info shadow-none bts-sm' data-bs-toggle='modal' data-bs-target='#room-images'>
                        <i class='bi bi-images'></i>
                    </button>
                    <button type='button' onclick='remove_room($row[id])' class='btn btn-danger shadow-none bts-sm'>
    ...
    if (isset($_POST['get_room_images'])) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            $thumb_btn = "<i class='bi bi-check-lg text-light bg-success px-2 py-1 rounded fs-5'></i>"; // Initialize $thumb_btn
    ...
    if (isset($_POST['rem_image'])) {
        ...
        $pre_q = "DELETE FROM `room_images` WHERE `sr_no`=? AND `room_id`=?";
        $res = delete($pre_q, $values, 'ii');  // Assuming your delete() function returns the result
