...
if (btn_submit) {
  btn_submit.onclick = async function (e) {
   ...
   // Store user's message in the conversation array with the user image
  conversation.push({ content: userMessage, sender: 'user', image: './images/UserIcon.png' });
  ...
  // Store chatbot's response in the conversation array with the chatbot image
  conversation.push({ content: response, sender: 'chatbot', image: './images/marvin.jpg', timestamp: getCurrTime()});
...
function setChatbox() {
  ...
  conversation.forEach(message => {
    const { content, sender, image } = message;
    const messageType = sender === 'user' ? 'user' : 'chatbot';
    const imageClass = messageType === 'user' ? 'user-image' : 'chatbot-image';
